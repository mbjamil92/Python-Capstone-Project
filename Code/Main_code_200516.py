...
def GPS():
  ...
  while True:
	...
	if len(msg.lat) > 0:
		...
		lat_one = lat_str[0:2]
		lat_two = lat_str[2:]
		lat_calc = ( float(lat_one) + float(lat_two)/60 )
		print lat_calc
		...
		lng_one = lng_str[0:2]
		lng_two = lng_str[2:]
		lng_calc = ( float(lng_one) + float(lng_two)/60 )
		print lng_calc        
...
class range_sensor(threading.Thread):
      def __init__(self,sensor ,ECHO, TRIG):
	threading.Thread.__init__(self)
	self.ECHO = ECHO
	self.TRIG = TRIG
	self.sensor = sensor
	print self.sensor," initialized"
      ...
      def run(self):
		...
		#self.right_distance = distance
        	...
        	print self.sensor, self.distance,"cm"    
